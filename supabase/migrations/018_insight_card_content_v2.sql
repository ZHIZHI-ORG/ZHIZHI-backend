-- Insight card content V2: one continuous body per source card. Both Large
-- and Medium details are provider-backed, idempotent, quota-bound artifacts.

ALTER TABLE insight_card_details
  ADD COLUMN IF NOT EXISTS provider_prompt_tokens BIGINT NOT NULL DEFAULT 0
    CHECK (provider_prompt_tokens >= 0),
  ADD COLUMN IF NOT EXISTS provider_billed_output_tokens BIGINT NOT NULL DEFAULT 0
    CHECK (provider_billed_output_tokens >= 0);

ALTER TABLE insight_card_follow_ups
  ADD COLUMN IF NOT EXISTS provider_prompt_tokens BIGINT NOT NULL DEFAULT 0
    CHECK (provider_prompt_tokens >= 0),
  ADD COLUMN IF NOT EXISTS provider_billed_output_tokens BIGINT NOT NULL DEFAULT 0
    CHECK (provider_billed_output_tokens >= 0);

ALTER TABLE insight_card_details DROP CONSTRAINT IF EXISTS insight_detail_quota_shape;
ALTER TABLE insight_card_details ADD CONSTRAINT insight_detail_quota_shape CHECK (
  (contract_version = 'insight_card_content_v1' AND (
    (source_type = 'medium' AND quota_day IS NOT NULL)
    OR (source_type = 'large' AND quota_day IS NULL)
  ))
  OR (contract_version = 'insight_card_content_v2' AND quota_day IS NOT NULL)
);

DROP INDEX IF EXISTS idx_insight_card_details_user_quota_day;
CREATE INDEX idx_insight_card_details_user_quota_day
  ON insight_card_details(user_id, quota_day)
...
BEGIN
  ...
  IF v_row.id IS NULL THEN
    ...
    ELSE
      ...
      WHERE id = p_source_batch_id AND user_id = p_user_id AND status = 'ready'
        AND prompt_version = 'recommendation_prompt_v8'
...
BEGIN
  ...
  v_body_length := char_length(p_detail_content_json->>'body');
  IF (v_row.source_type = 'large' AND v_body_length NOT BETWEEN 500 AND 800)
     OR (v_row.source_type = 'medium' AND v_body_length NOT BETWEEN 220 AND 420) THEN
    RETURN FALSE;
  END IF;
...
BEGIN
  IF jsonb_typeof(p_input_snapshot_json) <> 'object'
     ...
     OR jsonb_typeof(p_answer_content_json->'answer') <> 'string'
     OR char_length(p_answer_content_json->>'answer') NOT BETWEEN 80 AND 500
     OR jsonb_typeof(p_answer_content_json->'fact_refs') <> 'array'
     OR jsonb_array_length(p_answer_content_json->'fact_refs') NOT BETWEEN 1 AND 6 THEN
    RETURN FALSE;
  END IF;
  ...
  WHERE id = v_row.detail_id AND user_id = p_user_id AND status = 'ready'
    AND contract_version = 'insight_card_content_v2'
    AND output_schema_version = 'insight_card_detail_output_v2' FOR SHARE;
  ...
  UPDATE insight_card_follow_ups SET status = 'ready', lease_token = NULL,
    ...
    answer_json = jsonb_build_object(
      'follow_up_id', id, 'detail_id', detail_id,
      'parent_follow_up_id', parent_follow_up_id, 'question', question
    ) || p_answer_content_json,
    model_id = p_model_id, generation_metrics_json = p_generation_metrics_json,
    provider_attempt_count = provider_attempt_count
      + COALESCE((p_generation_metrics_json->>'provider_calls')::INTEGER, 0),
    provider_input_bytes = provider_input_bytes
...
CREATE OR REPLACE FUNCTION retry_insight_card_content_v2(
  ...
  p_provider_prompt_tokens BIGINT, p_provider_billed_output_tokens BIGINT
)
RETURNS BOOLEAN LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp AS $$
BEGIN
  IF p_kind = 'detail' THEN
    UPDATE insight_card_details SET status = 'retry_wait', lease_token = NULL,
      ...
      provider_prompt_tokens = provider_prompt_tokens + GREATEST(p_provider_prompt_tokens, 0),
      provider_billed_output_tokens = provider_billed_output_tokens
        + GREATEST(p_provider_billed_output_tokens, 0),
      last_error_code = p_error_code, last_error_stage = p_error_stage,
...
CREATE OR REPLACE FUNCTION fail_insight_card_content_v2(
  p_kind TEXT, p_generation_id UUID, p_user_id UUID, p_lease_token UUID,
  p_lease_epoch BIGINT, p_error_code TEXT, p_error_stage TEXT,
  p_provider_calls INTEGER, p_provider_input_bytes BIGINT,
  p_provider_prompt_tokens BIGINT, p_provider_billed_output_tokens BIGINT
)
RETURNS BOOLEAN LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp AS $$
BEGIN
...
REVOKE ALL ON FUNCTION claim_insight_card_detail(UUID, TEXT, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER, INTEGER)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION finalize_insight_card_detail(UUID, UUID, UUID, BIGINT, JSONB, TEXT, JSONB)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION finalize_insight_card_follow_up(UUID, UUID, UUID, BIGINT, JSONB, JSONB, TEXT, JSONB)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION retry_insight_card_content_v2(TEXT, UUID, UUID, UUID, BIGINT, INTEGER, TEXT, TEXT, INTEGER, BIGINT, BIGINT, BIGINT)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION fail_insight_card_content_v2(TEXT, UUID, UUID, UUID, BIGINT, TEXT, TEXT, INTEGER, BIGINT, BIGINT, BIGINT)
  FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION claim_insight_card_detail(UUID, TEXT, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER, INTEGER)
  TO service_role;
GRANT EXECUTE ON FUNCTION finalize_insight_card_detail(UUID, UUID, UUID, BIGINT, JSONB, TEXT, JSONB)
  TO service_role;
GRANT EXECUTE ON FUNCTION finalize_insight_card_follow_up(UUID, UUID, UUID, BIGINT, JSONB, JSONB, TEXT, JSONB)
  TO service_role;
GRANT EXECUTE ON FUNCTION retry_insight_card_content_v2(TEXT, UUID, UUID, UUID, BIGINT, INTEGER, TEXT, TEXT, INTEGER, BIGINT, BIGINT, BIGINT)
  TO service_role;
GRANT EXECUTE ON FUNCTION fail_insight_card_content_v2(TEXT, UUID, UUID, UUID, BIGINT, TEXT, TEXT, INTEGER, BIGINT, BIGINT, BIGINT)
  TO service_role;
